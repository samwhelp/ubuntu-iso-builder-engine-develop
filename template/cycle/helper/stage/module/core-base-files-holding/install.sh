...
cat << __EOF__ | tee /etc/apt/preferences.d/no-upgrade-base-files 2>&1 >/dev/null
Package: base-files
...
__EOF__
	...
	util_error_echo
	util_error_echo cat /etc/apt/preferences.d/no-upgrade-base-files
	util_error_echo
	cat /etc/apt/preferences.d/no-upgrade-base-files
